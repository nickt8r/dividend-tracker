...
const POSITIONS = {
  INDIV: [
    ...
    {tk:'NVDW', shares:300,  cost:11702.02, exDay:'MON'},
...
const SEED_PPS = {
  ...
  NVDW: [0.4690,0.3330,0.2830,0.3400,0.3500,0.3850,0.2910,0.3150,0.4250,0.1960,0.3380,0.3350,0.2130,0.2270,0.3630,0.3690,0.3840,0.3800,0.2090,0.4520,0.4820,0.2240,0.2650,0.2680],
...
function buildPortfolioData() {
  ...
  // Per-position cutoff — use purchase date if later than global cutoff
  const POSITION_CUTOFF = {
    'NVDW_300': new Date('2026-06-18'),
  };

  function getTotalDivs(tk, shares) {
    const key       = `${tk}_${shares}`;
    const seed      = SEED_DOLLARS[key] || 0;
    const posCutoff = POSITION_CUTOFF[key] || cutoff;
    const newPaid   = (yahoo[tk]?.divs || [])
      .filter(d => d.date >= posCutoff)
